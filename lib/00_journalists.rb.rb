...
#cCombien y a-t-il de journalistes dans ce array ?
...
puts  journalistes.count {|handle| handle.downcase.include? "aude"} 

#Combien commencent par une majuscule (première lettre juste après le @) ?
puts journalistes.grep(/^@\p{Upper}/).count

# Combien contiennent au moins une majuscule ?
puts journalistes.grep(/[[:upper:]]/).count

#combien ia t'il d'underscore
puts journalistes.grep(/\_/).count

#Trie la liste de handle par ordre alphabétique.
puts journalistes.sort { |a, b| a <=> b }

#Quels sont les 50 handles les plus courts de ce array ?
puts journalistes.sort_by(&:length)[0..49]
